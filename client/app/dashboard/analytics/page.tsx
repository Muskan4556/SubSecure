...
function StatCard({
  ...
  return (
    <div className="bg-[#0d1525] px-5 py-4">
      ...
      <div
        className={`text-[1.35rem] sm:text-[1.6rem] font-bold leading-none tracking-tight mb-1.5 ${valueColor}`}
      >
        {value}
      </div>
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/6 rounded-xl overflow-hidden border border-white/6">
          <StatCard
            label="Platform Monthly Spend"
            value={formatAmount(stats?.totalMonthlySpend ?? 0)}
          ...
          />
          <StatCard
            label="Total Cancelled Subscriptions"
            value={stats?.totalCancelled ?? 0}
            sub="historical"
          />
          <StatCard
            label="Upcoming Renewals (7 days)"
            value={renewals7?.data?.length ?? 0}
            sub="upcoming renewals"
            accent={(renewals7?.data?.length ?? 0) > 0 ? "amber" : undefined}
          />
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div className="rounded-xl overflow-hidden border border-white/6">
            <div className="flex items-center justify-between bg-[#0d1525] px-5 py-3.5 border-b border-white/4">
              <div>
                <p className="text-[13px] font-medium text-white/70">
                  Monthly plans
                </p>
                <p className="text-[11px] font-mono text-white/25 mt-0.5">
                  {allSubs?.data?.filter((s) => s.billingCycle === "MONTHLY")
                    .length ?? 0}{" "}
                  subscriptions
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="text-[11px] font-mono text-white/25">
                  {formatAmount(monthlyTotal * 12)}/yr
                </p>
              </div>
            </div>
            <div className="flex items-center justify-between bg-[#0d1525] px-5 py-3.5 border-b border-white/4">
              <div>
                ...
                <p className="text-[11px] font-mono text-white/25 mt-0.5">
                  {allSubs?.data?.filter((s) => s.billingCycle === "YEARLY")
                    .length ?? 0}{" "}
                  subscriptions
                </p>
              </div>
              <div className="text-right">
                <p className="text-[14px] font-mono text-white/80">
                  ...
                  <span className="text-[11px] text-white/25">/yr</span>
                </p>
                <p className="text-[11px] font-mono text-white/25">
                  {formatAmount(Math.round(yearlyTotal / 12))}/mo equiv.
                </p>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="rounded-xl overflow-hidden border border-white/6">
            {(allSubs?.data ?? []).slice(0, 6).map((sub) => (
              <div
                key={sub.id}
                className="flex items-center gap-3 bg-[#0d1525] px-5 py-2.5 border-b border-white/4 last:border-0"
              >
      ...
      {isAdmin && adminData && (
        <div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-white/6 rounded-xl overflow-hidden border border-white/6">
            <StatCard
              label="Total users"
              value={adminData.data.users.total}
              sub={`${adminData.data.users.active} active`}
            />
            <StatCard
              label="Suspended"
              value={adminData.data.users.suspended}
              sub="accounts"
              accent={adminData.data.users.suspended > 0 ? "red" : undefined}
            />
            <StatCard
              label="Total subscriptions"
              value={adminData.data.subscriptions.total}
              sub={`${adminData.data.subscriptions.active} active`}
            />
            <StatCard
              label="Audit events (24h)"
              value={adminData.data.security.auditEventsLast24h}
              sub="recent activity"
              accent="amber"
            />
          </div>
      ...
      <div className="flex items-center gap-2 text-[11px] font-mono text-white/20">
        ...
        <span>
          Spend is calculated based on active subscriptions. Yearly plans are
          normalised to monthly equivalents.
        </span>
      </div>
    </div>
  );
