    ...
    <div className="w-full rounded-xl overflow-hidden border border-white/7 shadow-[0_40px_120px_rgba(0,0,0,0.7)]">
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-[#0d1525] border-b border-white/6">
        ...
        <div className="flex items-center gap-1.5 text-white/20">
          ...
          <span className="text-[9px] font-mono">synced 1m ago</span>
        </div>
      </div>
      ...
      <div className="flex bg-[#0b1120]">
        ...
        <div className="hidden lg:flex w-[150px] shrink-0 bg-[#080e1a] border-r border-white/5 px-3 py-5 flex-col gap-1">
          <div className="flex items-center gap-2 px-2 py-2 mb-4">
            ...
            <span className="text-white/75 text-[11px] font-semibold">SubSecure</span>
          </div>

          {DASHBOARD_SIDEBAR_NAV.map(({ icon: Icon, label, active, badge }) => (
            <div
              key={label}
              className={`flex items-center justify-between px-2 py-1.5 rounded-md text-[10px] font-sans ${
            ...
            >
              ...
              {badge && (
                <span className="text-[8px] font-mono bg-white/10 text-white/40 px-1.5 rounded-full">
                  {badge}
                </span>
              )}
            </div>
          ...
          <div className="mt-auto pt-4 border-t border-white/5 px-2">
            ...
            <div className="text-[9px] font-mono text-white/25">alex@acme.co</div>
            <div className="text-[8px] font-mono text-emerald-500/60">ADMIN</div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <div className="flex flex-wrap items-start sm:items-center justify-between gap-2 px-4 sm:px-6 py-3 border-b border-white/5">
            <div>
              <div className="text-white/80 text-[12px] font-semibold">All Subscriptions</div>
            ...
            <div className="flex items-center gap-2">
              <div className="hidden sm:flex items-center gap-1.5 bg-amber-400/10 border border-amber-400/20 rounded-lg px-2.5 py-1.5">
                ...
                <span className="text-amber-300/80 text-[9px] font-sans">
                  Slack renewing in <strong>3 days</strong>
                </span>
              </div>
              <button className="bg-emerald-500/90 text-white text-[9px] font-sans px-2.5 py-1.5 rounded-lg">
                + Add new
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 border-b border-white/5">
            {DASHBOARD_STATS.map((s) => (
              ...
              >
                <div className="text-white/25 text-[8px] font-mono mb-1">{s.label}</div>
                <div className={`text-[13px] font-bold leading-none ${s.green ? "text-emerald-400" : "text-white/85"}`}>
                  {s.value}
                </div>
              </div>
            ))}
          ...
          {/* table header */}
          <div className="grid grid-cols-[1fr_auto_auto] sm:grid-cols-[1fr_auto_auto_auto_auto] px-4 sm:px-6 py-2 border-b border-white/5 text-[8px] font-mono text-white/20 uppercase tracking-wider gap-4 sm:gap-6">
            <span>Service</span>
          ...
          {DASHBOARD_ROWS.map((r, i) => (
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                />
                <span className="text-white/70 text-[10px] font-sans truncate">{r.name}</span>
              </div>
              <span className="hidden sm:block text-white/30 text-[9px] font-mono whitespace-nowrap">
                {r.plan}
              </span>
              <span className="text-white/60 text-[10px] font-mono text-right">{r.cost}</span>
              <div className="hidden sm:flex items-center gap-1 text-white/25 text-[9px] font-mono w-16 justify-end">
          ...
          <div className="px-4 sm:px-6 py-2.5 border-t border-white/5 bg-white/1">
            <div className="flex items-center gap-2 text-[9px] font-mono text-white/20 overflow-hidden">
