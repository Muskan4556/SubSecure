...
router.get("/approvals", requireAuth, requireRole(Role.ADMIN), getPendingApprovals);
...
router.put("/:id/approve",         requireAuth, requireRole(Role.ADMIN), approveSubscription);
...
router.put("/:id/schedule-cancel", requireAuth, requireRole(Role.USER), scheduleCancel);

// user only: CANCEL_SCHEDULED → ACTIVE  (undo before renewalDate is reached)
router.put("/:id/undo-cancel",     requireAuth, requireRole(Role.USER), undoScheduleCancel);
